  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (paymentQrImage?.startsWith("data:image"))
        return setQrCodeSrc(paymentQrImage);
      const str = paymentQrString || paymentQrUrl;
      if (str)
        try {
          setQrCodeSrc(await QRCode.toDataURL(str));
        } catch {}
    })();
  }, [mergedData, navigate]);
      ...
      <div className="min-h-screen flex items-center justify-center px-4">
        <div className="bg-gray-800/80 backdrop-blur-lg p-8 rounded-2xl border border-yellow-500/30 text-center max-w-lg w-full shadow-2xl">
          ...
          <div className="bg-gray-700/50 rounded-lg p-4 mb-6 text-left border border-gray-600">
            ...
            <p className="text-sm text-gray-300">
              Khách hàng:{" "}
              <span className="text-white">{mergedData?.customerName}</span>
            </p>
          </div>
          <p className="text-gray-300 mb-6 text-base leading-relaxed">
            Vui lòng liên hệ với Chạm qua Fanpage (kèm bill chuyển khoản) để
            Admin xác nhận và gửi vé cho bạn.
            <br />
            <br />
            <span className="text-sm text-yellow-400/90 italic">
              * Chúng tôi đang bảo trì hệ thống tự động, rất mong quý khách
              thông cảm.
            </span>
          </p>
          <div className="flex flex-col gap-3">
            ...
            <button
              onClick={() => setIsTransferred(false)} // Quay lại bước thanh toán bằng cách set lại state
              className="w-full py-3 bg-transparent border border-gray-500 text-gray-300 hover:text-white hover:border-white font-medium rounded-xl transition-all duration-200"
            >
      ...
      <div className="max-w-xl w-full bg-gray-800/50 backdrop-blur-lg p-8 rounded-xl border border-yellow-500/20">
        ...
        <div className="text-center">
          <img
            src={qrCodeSrc}
            alt="QR"
            className="mx-auto w-64 h-64 border-4 border-white rounded-lg mb-6"
          />
          ...
          <div className="bg-gray-700/50 p-6 rounded-lg mb-8 text-left">
            ...
            <p className="text-gray-300">
              Số lượng:{" "}
              <span className="text-white">{mergedData?.quantity} ghế</span>
            </p>
            <p className="text-2xl font-bold text-yellow-400 mt-2">
              Tổng: {mergedData?.totalPrice.toLocaleString("vi-VN")}đ
            </p>
          </div>
          <button
            onClick={() => setIsTransferred(true)}
            className="w-full py-4 bg-gradient-to-r from-yellow-500 to-orange-500 text-black font-bold rounded-lg shadow-lg"
          >
            Tôi đã chuyển khoản
          </button>
        </div>
